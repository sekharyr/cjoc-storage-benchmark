...
properties([
    parameters([
        choice(name: 'STORAGE_CLASS', choices: ['ebs-gp3-sc', 'efs-sc', 'openzfs-sc', 'openzfs-ha-sc'],
        ...
        string(name: 'STORAGE_RESOURCE_ID', defaultValue: '', description: 'The AWS resource ID backing THIS controller\'s JENKINS_HOME PVC — vol-xxxx for EBS, fs-xxxx for EFS/FSx. Leave blank to auto-select the right one for this fleet based on STORAGE_CLASS (see the hardcoded map in vars/benchStages.groovy) — set explicitly here only to override that. Requires the bench-agent pod to have AWS credentials (IRSA recommended) with cloudwatch:GetMetricStatistics.'),
    ...
    // Archived artifacts (bench-metrics.csv, trivy reports, and especially the
    // jars ARTIFACT_COPY_COUNT can multiply) accumulate on JENKINS_HOME forever
    // otherwise — keep the last 5 builds' worth, not unlimited history.
